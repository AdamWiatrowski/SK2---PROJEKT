...
void process_data(player &players, char *buffer, int bytes_read) {
    if (players.player_name[0] == '\0') {
        ...
        if (name_used) {
			...
			cout << "przypisano imie" << endl;
			cout << name << endl;
        }
    } else {
		cout << "imie juz przypisane" << endl;
        // nazwa już została przypisana, więc przetwarzaj wiadomość jako normalną wiadomość
    }
}
...
void start_game_if_host(player& current_player) {
    ...
    send_message_to_room(*(current_player.lobby), "GAME_STARTED");
}
